...
const { formatDatavaluePlain } = require( 'User:Zvpunry/repositories/DatavalueFormattingRepository.js' );
...
module.exports = {
	...
	setup(props, { emit } ) {
		...
		const calendarMenuItems = [
			{ label: i18n('valueview-expert-timevalue-calendar-julian'), value: 'http://www.wikidata.org/entity/Q1985786' },
			{ label: i18n('valueview-expert-timevalue-calendar-gregorian'), value: 'http://www.wikidata.org/entity/Q1985727' },
		];
		const precisionMenuItems = [
			...
			{ label: i18n('valueview-expert-timeinput-precision-year100k'), value: 4 },
			...
			{ label: i18n('valueview-expert-timeinput-precision-year100m'), value: 1 },
		...
		if ( props?.datavalue.value.time ) {
			formatDatavaluePlain( store.statementPropertyId, props?.datavalue ).then( ( data ) => {
		...
		function onInput (value) {
			...
			debouncedParseValue(value).then( ( { results, warnings } ) => {
				if ( warnings ) {
